...
router.get('/', auth, async (req: any, res: any) => {
  try {
    ...
    const averageServiceRating = feedbacks.filter((f: any) => f.serviceRating).length > 0
      ? feedbacks
          .filter((f: any) => f.serviceRating)
          .reduce((sum: number, f: any) => sum + f.serviceRating, 0) / 
        feedbacks.filter((f: any) => f.serviceRating).length
      : 0;

    const averageFoodRating = feedbacks.filter((f: any) => f.foodRating).length > 0
      ? feedbacks
          .filter((f: any) => f.foodRating)
          .reduce((sum: number, f: any) => sum + f.foodRating, 0) / 
